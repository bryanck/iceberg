...
public class IcebergSinkConfig extends AbstractConfig {
  ...
  @VisibleForTesting static final Pattern COMMA_REGEX = Pattern.compile(",");

  @VisibleForTesting
  static final Pattern COMMA_NO_PARENS_REGEX = Pattern.compile(",(?![^()]*+\\))");
  ...
  public TableSinkConfig tableConfig(String tableName) {
    return tableConfigMap.computeIfAbsent(
        ...
        notUsed -> {
          ...
          List<String> idColumns = stringToList(idColumnsStr, COMMA_REGEX);
  ...
  @VisibleForTesting
  static List<String> stringToList(String value, Pattern regex) {
    if (value == null || value.isEmpty()) {
      return ImmutableList.of();
    }

    return Splitter.on(regex).trimResults().splitToList(value);
  }
    ...
    if (javaCmd != null && !javaCmd.isEmpty()) {
      ...
      if (args.size() > 1
        ...
        try (InputStream in = Files.newInputStream(Paths.get(args.get(1)))) {
          ...
          LOG.warn("An error occurred reading worker properties", e);
        }
      }
    }
    LOG.info(
